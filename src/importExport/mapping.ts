...
const aliases: Record<NormalizedImportField, string[]> = {
  ...
  merchant: ["merchant", "counterparty", "店家"],
  item_name: ["item_name", "item", "名稱"],
  category: ["category", "分類", "種類"],
  source: ["source", "來源"],
  target_account: ["target_account", "transfer_account", "後帳戶"],
  target_amount: ["target_amount", "destination_amount", "目的金額"],
  target_currency: ["target_currency", "destination_currency", "目的幣別"],
  ...
  refund_linked_record_ids: ["refund_linked_record_ids", "關聯支出清單"],
  refund_excess_handling: ["refund_excess_handling", "退款差額處理"],
  reason: ["reason", "原因"],
