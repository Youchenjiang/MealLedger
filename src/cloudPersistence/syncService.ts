...
import { mapDraft, mapLedgerRecord } from "./mappers";
import { persistDraft, persistRecordBundle } from "./repository";
import {
  enqueueDraftSync,
  enqueueRecordSync,
...
export async function syncLocalChanges(input: SyncLocalChangesInput): Promise<SyncLocalChangesResult> {
  ...
  if (pending.length === 0) return { records: nextRecords, queue: nextQueue };
  ...
  if (!bootstrap.ok) {
    ...
    return { records: nextRecords, queue: nextQueue };
  }
