...
dev:5200`. Production email delivery (reset links, confirmation emails) uses
Resend as custom SMTP — see [Email delivery](email-delivery.md). The dev ports (4173, 4174, 5200) are registered in
...
URL Configuration → Redirect URLs, and set the **Site URL** there to the
deployed app so auth emails never link to a local address (see [Auth URL
Configuration](frontend-hosting.md#auth-url-configuration)). Creating the
OAuth clients, adding redirect URLs, and entering credentials in Supabase Auth
