...
- The normal entry flow is local-only. Magic Link is not the default or required sign-in method; the separate [auth spec](../specs/auth/requirements.md) selects Google OAuth, Facebook OAuth, or an ordinary email/password account.
...
- Provider choice is owned by the [auth spec](../specs/auth/requirements.md): Google OAuth, Facebook OAuth, or an ordinary email/password account.
