...
Added transfer-detail, transfer-detail deletion, refund-link, transfer-required, media-target ownership, and transfer-destination currency guards. Aligned the R2 thumbnail column with the canonical schema and verified temporary scan media links use canonical receipt/invoice intents.
