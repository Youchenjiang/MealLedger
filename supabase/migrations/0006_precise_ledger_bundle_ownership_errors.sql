-- Make ownership failures actionable while preserving the atomic bundle boundary.
-- This migration is intentionally diagnostic: it does not relax any policy.
...
begin
  ...
  if (p_ledger_record->>'category_id') is not null and not exists (
    ...
    raise exception 'ledger category is not owned by current user' using errcode = '42501';
  end if;
  if (p_ledger_record->>'merchant_id') is not null and not exists (
    ...
    raise exception 'ledger merchant is not owned by current user' using errcode = '42501';
  end if;
  if (p_ledger_record->>'event_id') is not null and not exists (
    ...
    raise exception 'ledger event is not owned by current user' using errcode = '42501';
  end if;

  if p_transfer_details is not null and p_transfer_details <> '{}'::jsonb then
    if (p_transfer_details->>'ledger_record_id')::uuid <> record_id then
      raise exception 'transfer ledger record does not match bundle record' using errcode = '42501';
    end if;
    if not exists (
      select 1 from public.accounts
    ...
    if (p_transfer_details->>'fee_ledger_record_id') is not null and not exists (
      ...
      raise exception 'transfer fee record is not owned by current user' using errcode = '42501';
    end if;
  end if;
  ...
  if exists (
    ...
    raise exception 'refund link does not match bundle record' using errcode = '42501';
  end if;
  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_refund_links, '[]'::jsonb)) as link
    ...
    raise exception 'refund original record is not owned by current user' using errcode = '42501';
  end if;

  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_ledger_record_tags, '[]'::jsonb)) as tag
    where (tag->>'user_id')::uuid <> request_user
  ) then
    raise exception 'ledger tag user does not match authenticated user' using errcode = '42501';
  end if;
  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_ledger_record_tags, '[]'::jsonb)) as tag
    where (tag->>'ledger_record_id')::uuid <> record_id
  ) then
    raise exception 'ledger tag does not match bundle record' using errcode = '42501';
  end if;
  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_ledger_record_tags, '[]'::jsonb)) as tag
    ...
    raise exception 'ledger tag is not owned by current user' using errcode = '42501';
  end if;

  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_audit_events, '[]'::jsonb)) as audit
    where (audit->>'user_id')::uuid <> request_user
  ) then
    raise exception 'audit event user does not match authenticated user' using errcode = '42501';
  end if;
  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_audit_events, '[]'::jsonb)) as audit
    ...
    raise exception 'audit event does not match bundle record' using errcode = '42501';
  end if;
