-- Resolve stale local account ids only within the authenticated user's accounts.
-- The wrapper preserves the existing atomic RPC and does not relax RLS ownership.
...
begin
  ...
  if not exists (select 1 from public.accounts where id = source_account_id and user_id = request_user) then
    ...
    where user_id = request_user
      and lower(trim(name)) = lower(trim(p_ledger_record->>'account_name'))
  ...
  if p_transfer_details is not null and p_transfer_details <> '{}'::jsonb then
    ...
    if not exists (select 1 from public.accounts where id = destination_account_id and user_id = request_user) then
      ...
      from public.accounts
      where user_id = request_user
        and lower(trim(name)) = lower(trim(p_transfer_details->>'destination_account_name'))
      order by created_at, id
      limit 1;
