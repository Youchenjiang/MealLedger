-- Keep the atomic transfer write behind one authenticated, ownership-checked
-- boundary. The function must not depend on child-table RLS while it writes
-- the bundle, but every referenced row is still checked against auth.uid().
...
begin
  ...
  if exists (
    ...
    where (link->>'refund_record_id')::uuid <> record_id
      or not exists (select 1 from public.ledger_records where id = (link->>'original_record_id')::uuid and user_id = request_user)
  ) then
    raise exception 'refund bundle references data owned by another user' using errcode = '42501';
  end if;
  ...
  if exists (
    ...
    where (tag->>'user_id')::uuid <> request_user
      or (tag->>'ledger_record_id')::uuid <> record_id
      or not exists (select 1 from public.tags where id = (tag->>'tag_id')::uuid and user_id = request_user)
  ) then
    raise exception 'tag bundle references data owned by another user' using errcode = '42501';
  end if;
  ...
  if exists (
    ...
    where (audit->>'user_id')::uuid <> request_user
      or (audit->>'target_type') = 'ledger-record' and (audit->>'target_id')::uuid <> record_id
  ) then
    raise exception 'audit bundle references data owned by another user' using errcode = '42501';
  end if;
